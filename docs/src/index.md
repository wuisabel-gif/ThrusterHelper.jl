...
- **Analysis**: `reachable`, `compare_methods`, `rank_failures`, `monte_carlo`, `mission_energy`.
- **Design**: `optimize_layout` and `sensitivity`: re-aim/move thrusters to improve the design.
- **Dynamics**: `simulate` (closed-loop PID), `top_speeds`, thrust curves (`ThrustCurve`).
- **Shared budgets**: `estimate_current`, `group_totals`, `allocate_grouped` for power-board current limits.
- **Deploy**: `export_cpp` (a dependency-free C++17 header) and a ROS 2 node wrapper.
